 ...
 * 
 * 처음 아이디어 : 
 * 플로이드 와샬 알고리즘 사용!! => BUT N의 제한이 1000이므로 n^3떄리면 10억이 넘어간다!!
 * 
 * 다음 아이디어 : 
 * 다익스트라 알고리즘 사용
 * 왕복 비용 값을 구해야함
 * 
 * 도착지가 X라고 할때, X에서 모든 도착지로 다익스트라 실행
 * 각 국가에서 다익스트라를 실행해 X까지의 시간을 구한다. => 여기가 문제!!
 * !!!핵심!!!
 * 이부분에서 X를 제외한 국가에서 X까지 소요되는 시간을 모두 다익스트라로 돌리면 필요없는 값들도 들어옴
 * => 단방향 그래프를 반대로 입력받아 X까지의 소요되는 시간을 측정한다.(WOOOOOOWWW!!!) => 생각지도 모했음 
 * 
 * 
 * 
 * 
...
public class Main_BOJ_1238_파티_Gold3_184ms {
